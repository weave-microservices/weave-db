  ...
  return {
    ...
    sanitizeParams (data, options) {
      ...
      if (sanitizedData.mapIds === true) {
        if (
          Array.isArray(sanitizedData.fields) &&
          sanitizedData.fields.length > 0 &&
          !sanitizedData.fields.includes(this.settings.idFieldName)
        ) {
    ...
    async insert (context, insertEntity) {
      const { entity } = context.data; // await this.validateEntity(context.data.entity);
    ...
    async insertMany (context) {
      const { entities } = context.data; // await Promise.all(context.data.entities.map((entity) => this.validateEntity(entity)));
